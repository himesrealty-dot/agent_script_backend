...
// Returns { hook, say, caption }. Uses the Railway backend if a Backend URL is set;
// otherwise falls back to calling Anthropic directly with an on-device key.
async function generateScript(idea){
  const model = load(LS.model,'claude-haiku-4-5-20251001');
  const backend = (load(LS.backend,'')||'').trim().replace(/\/+$/,'');
  ...
  // Direct-to-Anthropic fallback (BYO key on device)
...
function hasAuth(){ return !!((load(LS.backend,'')||'').trim() || (load(LS.key,'')||'').trim()); }
